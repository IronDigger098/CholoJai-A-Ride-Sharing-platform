...
| `TRUSTED_PROXY_HOPS`             | Reverse proxies to trust for XFF                    | `0` (2 via Vercel → Render)    | M3        |
...
| `NEXT_PUBLIC_SOCKET_URL`   | Socket.IO origin (the API's real host)                | derived from `NEXT_PUBLIC_API_BASE_URL` | M12       |
| `API_PROXY_TARGET`         | Build-time: where `/api/v1/*` is proxied (not public) | unset (no proxy)                        | M12       |
...
### Production values for the web app

`NEXT_PUBLIC_API_BASE_URL=/api/v1` (relative) together with
`API_PROXY_TARGET=https://<api host>` routes REST calls through the web
app's own origin, so the `SameSite=Strict` refresh cookie is first-party.
`COOKIE_DOMAIN` on the API must then be the **web** host, because that is
the host the browser sees setting it. Sockets cannot go through the proxy,
so `NEXT_PUBLIC_SOCKET_URL=https://<api host>` points them at the API
directly; they authenticate with the access token, not the cookie.
